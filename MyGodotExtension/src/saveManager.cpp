...
void SaveManager::save_game(int current_room_id) {
    ...
    Node *player_status_node =
        get_node_or_null(NodePath("/root/PlayerStatusData"));

    PlayerStatus *player_status =
        Object::cast_to<PlayerStatus>(player_status_node);

    if (player_status == nullptr) {
        UtilityFunctions::print(
            "SAVE ERROR: Cannot find PlayerStatusData.");
        return;
    }
    ...
    if (save_file.is_null()) {
        UtilityFunctions::print(
            "SAVE ERROR: Could not open save_game.json.");
        return;
    }
